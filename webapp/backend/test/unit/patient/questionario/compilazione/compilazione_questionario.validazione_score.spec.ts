...
describe('Compilazione_questionarioService - Unit 3: Validazione Score', () => {
    let service: Compilazione_questionarioService;
    const oracle = loadOracle('patient/questionario/compilazione/compilazione-unit3');
    ...
    const testCases = ['TC_RF3_8', 'TC_RF3_9', 'TC_RF3_10', 'TC_RF3_11'];
    ...
    testCases.forEach(id => {
        ...
        it(`${testCase.id}: ${testCase.description}`, async () => {
            ...
            if (mockData.tipologiaExists !== undefined) {
                ((db as any).limit as jest.Mock).mockResolvedValue(mockData.tipologiaExists ? [{ id: '1' }] : []);
            }

            try {
                const result = await service.validazioneCalcoloScore(input.nomeTipologia, input.risposte);
